    ...
    char (*Compare)(void *, uint16_t, void *, uint16_t), void (*Free)(void *))
{
    ...
    /* setup the filter */
    ht = UTIL_MALLOC(sizeof(HashTable));
    if (unlikely(ht == NULL))
        goto error;
    ...
    /* setup the bitarray */
    ht->array = UTIL_MALLOC(ht->array_size * sizeof(HashTableBucket *));
    if (ht->array == NULL)
        goto error;
...
int HashTableAdd(HashTable *ht, void *data, uint16_t datalen)
{
    ...
    HashTableBucket *hb = UTIL_MALLOC(sizeof(HashTableBucket));
    if (unlikely(hb == NULL))
        goto error;
                             ...
                             uint16_t len2)
{
    ...
    if (UTIL_MEMCMP(data1, data2, len1) != 0)
        return 0;
