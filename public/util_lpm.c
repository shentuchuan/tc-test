...
int __dns_lpm_set_ip_policy(uint16_t busi_type, uint8_t op,
    ...
    list_for_each_entry(dns_lpm_policy_data,
                        ip->is_ipv6 == IS_IPV4 ? &(dns_lpm_node->config_list_ipv4) : &(dns_lpm_node->config_list_ipv6),
                        list) {
...
void __dns_lpm_get_ip_policy(uint8_t is_ipv6, unsigned char *ip,
    ...
    list_for_each_entry(current_ip_info,
                        is_ipv6 == IS_IPV4 ? &(dns_lpm_node->config_list_ipv4) : &(dns_lpm_node->config_list_ipv6),
                        list) {
...
void _dns_lpm_get_ip_policy(uint8_t is_ipv6, unsigned char *ip,
    ...
    if (hash_node == NULL) { /*dns_lpm_node 表示上层到此的入口，dns_lpm_node
                                表示本层具体的实体*/
        return;
    }
    ...
    _dns_lpm_get_ip_policy(
        ...
        level + 1); // 递归往深处level走 刷新ip_info  掩码越大 优先级越高 (如某一配置
    ...
    if (filter == NULL) {
        ...
        if ((level + 1) * 8 < filter->mask && tmp_hash_head->dns_lpm_node != NULL) {
            ...
            for (i = 0; i <= extra; i++) {
                ...
                list_for_each_entry(current_ip_info,
                                    filter->is_ipv6 == IS_IPV4 ? &(tmp_hash_head->config_list_ipv4) : &(tmp_hash_head->config_list_ipv6),
                                    list) {
