...
void ipv6_addr_prefix(struct in6_addr *pfx, const struct in6_addr *addr, int plen) {
...
int inet_ntop4(const unsigned char *src, unsigned char *dst, unsigned char size) {
...
int inet_ntop6(const unsigned char *src, unsigned char *dst, unsigned char size) {
    /*
    ...
    unsigned char tmp[sizeof "ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255"], *tp;
    struct {
        int base, len;
    } best = {-1, 0}, cur = {-1, 0};
...
dns_lpm_node_t *alloc_single_hash_level(void) {
    ...
    single_hash_head = (dns_lpm_node_t *)UTIL_MALLOC(sizeof(dns_lpm_node_t) * (MAX_INDEX + 1));
    if (!single_hash_head) {
...
int dns_lpm_set_ip_policy_info(uint16_t result_type, uint8_t op, lpm_user_result_t *data, dns_lpm_policy_data_t *dns_lpm_policy_data, ip_any_t *ip) {
...
int __dns_lpm_set_ip_policy(uint16_t busi_type, uint8_t op, lpm_user_result_t *data, dns_lpm_node_t *dns_lpm_node, ip_any_t *ip) {
    ...
    // 已经存在节点
    list_for_each_entry(dns_lpm_policy_data, ip->is_ipv6 == IS_IPV4 ? &(dns_lpm_node->config_list_ipv4) : &(dns_lpm_node->config_list_ipv6), list) {
        if (dns_lpm_policy_data->mask == ip->mask) {
            ret = dns_lpm_set_ip_policy_info(busi_type, op, data, dns_lpm_policy_data, ip);
            if (op == OP_DEL) {
    ...
    // 还未存在
    dns_lpm_policy_data_new = (dns_lpm_policy_data_t *)UTIL_MALLOC(sizeof(dns_lpm_policy_data_t));
    ...
    ret = dns_lpm_set_ip_policy_info(busi_type, op, data, dns_lpm_policy_data_new, ip);
    if (ret < 0) {
    ...
    list_for_each_entry(dns_lpm_policy_data, ip->is_ipv6 == IS_IPV4 ? &(dns_lpm_node->config_list_ipv4) : &(dns_lpm_node->config_list_ipv6), list) {
        if (ip->mask > dns_lpm_policy_data->mask) {
            list_add(&dns_lpm_policy_data_new->list, &dns_lpm_policy_data->list); // 掩码大的在后 优先级高
            goto out;
        }
    }
    list_add(&dns_lpm_policy_data_new->list, ip->is_ipv6 == IS_IPV4 ? &(dns_lpm_node->config_list_ipv4) : &(dns_lpm_node->config_list_ipv6));

out:
    return ret;
}

int _dns_lpm_set_ip_policy(uint16_t busi_type, uint8_t op, ip_any_t *ip, lpm_user_result_t *data, dns_lpm_node_t *dns_lpm_node, uint8_t level) {
...
int dns_lpm_set_all(uint16_t busi_type, uint8_t op, ip_any_t *ip, lpm_user_result_t *data, dns_lpm_hlist_t *p) {
    ipaddr_prefix_recalc(ip);
    // dns_lpm_set_debug(busi_type, op, ip, data);
    if (_dns_lpm_set_ip_policy(busi_type, op, ip, data, &p->head /*hash 入口*/, 0)) {
...
bool dns_lpm_update(uint8_t op, uint16_t busi_type, lpm_user_result_t *data, ip_any_t *ip, dns_lpm_hlist_t *hlist_new) {
    if (op == 0 && hlist_new != NULL) {
        if (dns_lpm_set_all(busi_type, OP_ADD, ip, data, hlist_new) < 0) {
            return false;
        }
    } else {
        util_log_error("dns lpm load file invalid para. op: %u, hlist_new: %px", op, hlist_new);
        return false;
    }
...
int dns_lpm_table_addrs_group_add(dns_lpm_hlist_t *hlist_new, ip_any_t *ip, lpm_user_result_t *data) {
    return dns_lpm_set_all(LPM_RESULT_TYPE_ID, OP_ADD, ip, data, hlist_new);
}

void dns_lpm_get_ip_policy_info(dns_lpm_policy_data_out_t *out, dns_lpm_policy_data_t *in) {
    if (out == NULL || in == NULL) {
    ...
    if (out->user_result_cnt >= UTIL_LPM_MAX_MATCH) {
        util_log_error("dns_lpm_get_ip_policy_info out->user_result_cnt >= MAX_USER_RESULT");
        return;
    }
...
void __dns_lpm_get_ip_policy(uint8_t is_ipv6, unsigned char *ip, dns_lpm_node_t *dns_lpm_node, dns_lpm_policy_data_out_t *dns_lpm_policy_data) {
    dns_lpm_policy_data_t *current_ip_info = NULL;

    list_for_each_entry(current_ip_info, is_ipv6 == IS_IPV4 ? &(dns_lpm_node->config_list_ipv4) : &(dns_lpm_node->config_list_ipv6), list) {
        dns_lpm_get_ip_policy_info(dns_lpm_policy_data, current_ip_info);
    }
    return;
}

void _dns_lpm_get_ip_policy(uint8_t is_ipv6, unsigned char *ip, dns_lpm_node_t *dns_lpm_node, dns_lpm_policy_data_out_t *dns_lpm_policy_data, uint8_t level) {
    dns_lpm_node_t *tmp_hash_head = NULL;
    ...
    if (hash_node == NULL) { /*dns_lpm_node 表示上层到此的入口，dns_lpm_node 表示本层具体的实体*/
        return;
    }

    tmp_hash_head = &(hash_node[ip[level]]);
    __dns_lpm_get_ip_policy(is_ipv6, ip, tmp_hash_head, dns_lpm_policy_data);           // 获取当前level 的配置
    _dns_lpm_get_ip_policy(is_ipv6, ip, tmp_hash_head, dns_lpm_policy_data, level + 1); // 递归往深处level走 刷新ip_info  掩码越大 优先级越高 (如某一配置 更深level有 则更新，如果没，则保持当前level的配置)

    return;
}

void dns_lpm_table_match(dns_lpm_hlist_t *p, uint8_t is_ipv6, uint8_t *ip, dns_lpm_policy_data_out_t *dns_lpm_policy_data_out) {
    int i = 0;
    _dns_lpm_get_ip_policy(is_ipv6, ip, &p->head /*hash 入口*/, dns_lpm_policy_data_out, 0);

    util_log_debug("user_result_cnt:%d", dns_lpm_policy_data_out->user_result_cnt);
    for (i = 0; i < dns_lpm_policy_data_out->user_result_cnt; i++) {
        util_log_debug("    result :%d, user_result_id:%ld", i, dns_lpm_policy_data_out->user_result[i].user_result_id);
    }

    return;
}

static void dns_lpm_policy_print(dns_lpm_policy_data_t *data, ip_any_t *ip, uint8_t is_ipv6) {
    char ip_str[64] = {0};
    ...
    if (is_ipv6 == IS_IPV4) {
        inet_ntop4((unsigned char *)&ip->ipv4, (unsigned char *)ip_str, sizeof(ip_str));
    } else {
        inet_ntop6((unsigned char *)&ip->ipv6, (unsigned char *)ip_str, sizeof(ip_str));
    }
    util_log_debug("dns_lpm_policy_print ip:%s, mask:%u, user_result_id:%ld", ip_str, data->mask, data->user_result.user_result_id);
}

void dns_lpm_print(dns_lpm_node_t *dns_lpm_node, ip_any_t *ip_stack, uint8_t level, ip_query_protocol_t *filter) {
    uint16_t i, index;
    uint8_t  extra = 0;
    bool     is_default_ipv4_printed, is_default_ipv6_printed; // 用于避免mask=0的规则展开打印
    ...
    if (filter == NULL) {
        for (i = 0; i <= MAX_INDEX; i++) {
            ...
            // util_log_debug("dns_lpm_print level: %u, index: %u, hash head: %px", level, i, hash_node);

            if (level < MAX_LEVEL_IPV4) {
                list_for_each_entry(current_ip_info, &tmp_hash_head->config_list_ipv4, list) {
                    if (current_ip_info->mask == 0 && is_default_ipv4_printed == true) {
                        continue;
                    } else if (current_ip_info->mask == 0 && is_default_ipv4_printed == false) {
            ...
            list_for_each_entry(current_ip_info, &tmp_hash_head->config_list_ipv6, list) {
                if (current_ip_info->mask == 0 && is_default_ipv6_printed == true) {
                    continue;
                } else if (current_ip_info->mask == 0 && is_default_ipv6_printed == false) {
            ...
            memset(ip_stack->ip_byte + level + 1, 0, sizeof(ip_stack->ip_byte) - level - 1);
        }
    } else {
        ...
        if ((level + 1) * 8 < filter->mask && tmp_hash_head->dns_lpm_node != NULL) {
            ...
            for (i = 0; i <= extra; i++) {
                ...
                list_for_each_entry(current_ip_info,
                                    filter->is_ipv6 == IS_IPV4 ? &(tmp_hash_head->config_list_ipv4) :
